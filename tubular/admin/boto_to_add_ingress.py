...
def add_ingress_rule(dry_run, go_agent_security_group, go_agent_security_group_owner, go_agent_security_group_name):
    ...
    """
    asg_conn = boto3.client('autoscaling', region_name='us-east-1')
    ...
    go_agent_security_group = ec2.SecurityGroup(
    ...
    for group in asgs:
        ...
        try:
            ...
            logging.error("Security group '{}' for ASG '{}' was not found!.".format(
                sg_name, group.name))
        logging.info(
            'BEFORE: Rules for security group {}:'.format(sec_group.name))
        logging.info(sec_group.rules)
        try:
            # Add the ingress rule to the security group.
            yes_no = six.moves.input(
                "Apply the change to this security group? [Yes]")
